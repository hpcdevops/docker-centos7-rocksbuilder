...
for f in $(find . -name "*.iso"); do isoinfo -R -f -i $f ; done

popd

echo "=-=-=- Build of ${ROLL_TO_BUILD} required "$(du -s --block-size=1k ${ROLL_TO_BUILD} | awk '{printf "%'\''d KB\n", $1}')" -=-=-="

pushd ${ROLL_TO_BUILD}

for f in $(find . -name "*.iso"); do mv -v $f ../ ; done
